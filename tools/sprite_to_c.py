...
def encode_spritesheet(writer, name: str, content: str, optimized: bool) -> str:
    ...
    for item in obj:
        ...
        writer.write(f"static unsigned char {symbol_name}[] = {{\n")
        writer.write(f"    {width}, {height}, {xPivot}, {yPivot},\n")
        for row in rows:
            line = ""
        ...
        if padding == 2:
            if "padding" in item:
                ...
                padding_data = 0
            writer.write(f"    {padding_data}, {padding_data}\n")
        writer.write("};\n")

    # Create a list of sprites
    writer.write(f"unsigned char* {sym_name}[] = {{\n")
    for i in range(0, len(obj)):
        writer.write(f"    {syms[i]},\n")
    writer.write("};\n")
...
def decode_spritesheet(out_path, data: bytearray, start: int, palette) -> list:
    ...
    items = []
    while True:
        raw_off = to_u32(data[start + len(items) * 4 :])
        ...
        if padding == 2:
            padding_data = to_u16(sprite_data[4 + sprite_byte_count :])
            if padding_data != 0:
